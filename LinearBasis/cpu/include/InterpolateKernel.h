	...
	const T& getFunc()
	{
		...
		if (!func)
		{
			...
			if (!func)
			{
				...
				process->cout("Rank #%d loaded %s kernel for dim = %d, count = %d, DofPerNode = %d : %s @ 0x%x\n",
					process->getRank(), type.c_str(), dim, count, DofPerNode, funcname.c_str(), (void*)func);
			}
			PTHREAD_ERR_CHECK(pthread_mutex_unlock(&mutex));
		}
