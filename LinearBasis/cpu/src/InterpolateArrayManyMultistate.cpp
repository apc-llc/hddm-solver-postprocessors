...
static vector<vector<int> > ind_I_;
static vector<vector<int> > ind_J_;
static vector<vector<int> > rowinds_;
	...
	const Matrix<int>* index_, const Matrix<double>* surplus_, double** value_)
{
	...
	for (int many = 0; many < count; many++)
	{
		...
		if (!initialized)
		{
			...
			ind_I_.resize(count);
			ind_J_.resize(count);
			rowinds_.resize(count);
		
			for (int many = 0; many < count; many++)
			{
				vector<int>& ind_I = ind_I_[many];
				vector<int>& ind_J = ind_J_[many];
				vector<int>& rowinds = rowinds_[many];
