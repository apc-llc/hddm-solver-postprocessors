	...
	const Matrix<int>::Device* index, const Matrix<double>::Device* surplus, double* value)
{
	...
	real* dvalue;
	CUDA_ERR_CHECK(cudaMalloc(&dvalue, sizeof(real) * length));
	
	cudaStream_t stream;
	CUDA_ERR_CHECK(cudaStreamCreate(&stream));
	CUDA_ERR_CHECK(cudaMemcpyAsync(dx, x, sizeof(real) * DIM, cudaMemcpyHostToDevice, stream));
	CUDA_ERR_CHECK(cudaMemsetAsync(dvalue, 0, sizeof(real) * length, stream));
	InterpolateArray_kernel_large_dim<<<gridDim, blockDim, 0, stream>>>(
		dim, vdim, nno, Dof_choice_start, Dof_choice_end, dx, index, surplus, dvalue);
	CUDA_ERR_CHECK(cudaGetLastError());
	CUDA_ERR_CHECK(cudaMemcpyAsync(value, dvalue, sizeof(real) * length, cudaMemcpyDeviceToHost, stream));
