...
static __global__ void InterpolateArrayManyMultistate_kernel_large_dim(
	...
	const Matrix<int>::Device* index, const Matrix<double>::Device* surplus, double** value)
{
	...
	InterpolateArrayManyMultistate_kernel_large_dim<<<gridDim, blockDim, sizeof(double) * nwarps, stream>>>(
