...
struct AVXIndex_
{
	...
	{
		AVXIndex_ empty;
		if (memcmp(this, &empty, sizeof(AVXIndex_)) == 0)
			return true;
...
class AVXIndexes: private std::vector<char, AlignedAllocator<AVXIndex_> >
{
	int nnoMax;
	int dim;
...
public :
	...
	AVXIndexes() :
		nnoMax(0), dim(0),
		std::vector<char, AlignedAllocator<AVXIndex_> >()
	...
	AVXIndexes(int nnoMax_, int dim_) :
		...
		std::vector<char, AlignedAllocator<AVXIndex_> >(
			dim_ * (nnoMaxAlign(nnoMax_) * sizeof(AVXIndex_) + szlength))
	...
	void resize(int nnoMax_, int dim_)
	{
		...
		vector<char, AlignedAllocator<AVXIndex_> >::resize(
			dim_ * (nnoMaxAlign(nnoMax_) * sizeof(AVXIndex_) + szlength));
	}
	
	__attribute__((always_inline))
	AVXIndex_& operator()(int i, int j)
	{
		return *reinterpret_cast<AVXIndex_*>(
			&std::vector<char, AlignedAllocator<AVXIndex_> >::operator[]((j * nnoMax + i) * sizeof(AVXIndex_) + (j + 1) * szlength));
	}

	__attribute__((always_inline))
	const AVXIndex_& operator()(int i, int j) const
	{
		return *reinterpret_cast<const AVXIndex_*>(
			&std::vector<char, AlignedAllocator<AVXIndex_> >::operator[]((j * nnoMax + i) * sizeof(AVXIndex_) + (j + 1) * szlength));
	}
	...
	void calculateLengths()
	{
		for (int j = 0; j < dim; j++)
		{
			...
			{
				AVXIndex_& index = this->operator()(i, j);
				if (index.isEmpty())
					break;
...
void DataSparse::load(const char* filename, int istate)
{
	...
	for (int ifreq = 0; ifreq < state.nfreqs; ifreq++)
	{
		...
		for (int j = 0, length = indexes.size() / dim / AVX_VECTOR_SIZE; j < dim; j++)
		{
			...
			{
				AVXIndex_& index = avxindsFreq(i, j);
	...
	for (int ifreq = 0, ixp = 1; ifreq < state.nfreqs; ifreq++)
	{
		...
		for (int j = 0, itemp = 0; j < dim; j++)
		{
			...
			{
				const AVXIndex_& avxind = freq.avxinds(i, j);
			...
			{
				const AVXIndex_& index = freq.avxinds(freq.avxinds.getLength(j) - 1, j);
	...
	freq.xps.resize((map.xps.size() / DOUBLE_VECTOR_SIZE));
	if (map.xps.size() % DOUBLE_VECTOR_SIZE)
		freq.xps.resize(freq.xps.size() + 1);
	freq.xps.resize(freq.xps.size() * DOUBLE_VECTOR_SIZE);
	for (std::map<Index<uint16_t>, uint32_t>::const_iterator i = map.xps.begin(), e = map.xps.end(); i != e; i++)
		freq.xps[i->second / DOUBLE_VECTOR_SIZE].assign(i->first, i->second % DOUBLE_VECTOR_SIZE);
