...
struct AVXIndex
{
	TIndex index[DOUBLE_VECTOR_SIZE];
	int8_t i[DOUBLE_VECTOR_SIZE];
	int8_t j[DOUBLE_VECTOR_SIZE];

	AVXIndex()
	{
		memset(index, 0, DOUBLE_VECTOR_SIZE * sizeof(TIndex));
		memset(i, 0, DOUBLE_VECTOR_SIZE * sizeof(int8_t));
		memset(j, 0, DOUBLE_VECTOR_SIZE * sizeof(int8_t));
	}

	void assign(const Index<TIndex>& ind, int pos)
	{
		index[pos] = ind.index;
		i[pos] = ind.i;
		j[pos] = -ind.j; // negated to simplify AVX FMA
	}
};
